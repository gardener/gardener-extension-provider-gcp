...
`)
			err := workloadIdentityValidator.Validate(ctx, newWorkloadIdentity, workloadIdentity)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(`validation of target system's configuration failed: spec.targetSystem.providerConfig.credentialsConfig.token_url: Invalid value: "https://sts.googleapis.com/v1/token-forbidden": should be one of the allowed URLs: https://sts.googleapis.com/v1/token, https://sts.googleapis.com/v1/token/new`))
		})
