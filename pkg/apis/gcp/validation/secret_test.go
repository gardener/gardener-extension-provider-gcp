...
var _ = Describe("Secret validation", func() {
	...
	DescribeTable("#ValidateCloudProviderSecret",
		...
		Entry("should return error when the serviceaccount.json field is empty",
			map[string][]byte{gcp.ServiceAccountJSONField: []byte(``)}, HaveOccurred()),
		Entry("should return error when the serviceaccount.json field is not a valid json",
			map[string][]byte{gcp.ServiceAccountJSONField: []byte(`{invalid}`)}, HaveOccurred()),
