...
func (fctx *FlowContext) Reconcile(ctx context.Context) error {
	...
	subnetIPv6CidrBlock := fctx.whiteboard.Get(ServicesSubnetIPv6Cidr)

	return PatchProviderStatusAndState(ctx, fctx.runtimeClient, fctx.infra, fctx.networking, status, state, subnetIPv6CidrBlock)
}
...
func (fctx *FlowContext) persistState(ctx context.Context) error {
	...
	return PatchProviderStatusAndState(ctx, fctx.runtimeClient, fctx.infra, fctx.networking, nil, fctx.getCurrentState(), subnetIPv6CidrBlock)
}
...
func PatchProviderStatusAndState(
	...
	if status != nil {
		...
		if subnetIPv6CIDR != nil {
			infra.Status.Networking.Services = append(infra.Status.Networking.Services, *subnetIPv6CIDR)
		}
	}
