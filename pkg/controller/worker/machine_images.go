...
func (w *WorkerDelegate) selectMachineImageForWorkerPool(name, version string, arch *string, machineCapabilities gardencorev1beta1.Capabilities) (*api.MachineImage, error) {
	selectedMachineImage := &api.MachineImage{
		Name:         name,
		Version:      version,
		Architecture: arch,
	}
	if capabilitySet, err := helper.FindImageInCloudProfile(w.cloudProfileConfig, name, version, arch, machineCapabilities, w.cluster.CloudProfile.Spec.MachineCapabilities); err == nil {
	...
	return nil, worker.ErrorMachineImageNotFound(name, version, *arch)
}
